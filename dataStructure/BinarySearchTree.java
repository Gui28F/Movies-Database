...
import dataStructure.exceptions.EmptyDictionaryException;
...
public class BinarySearchTree<K extends Comparable<K>, V>
    ...
    /**
     * Tree Constructor - creates an empty tree with a custom comparator
    ...
    protected BSTNode<K, V> findNode(BSTNode<K, V> node, K key) {
        if (node == null)
            ...
            int compResult = this.compare(key, node.getKey());
            if (compResult == 0)
                return node;
    ...
    public boolean insertNode(BSTNode<K,V> subTreeRoot, BSTNode<K,V> newNode){
        if(subTreeRoot.getKey().equals(newNode.getKey())) return false;
        if(subTreeRoot.getKey().compareTo(newNode.getKey()) > 0)
            if(subTreeRoot.getLeft() == null) {
                subTreeRoot.setLeft(newNode);
                return true;
            }else
                return insertNode(subTreeRoot.getLeft(), newNode);
        else
            if(subTreeRoot.getRight() == null) {
                subTreeRoot.setRight(newNode);
                return true;
            }else
                return insertNode(subTreeRoot.getRight(), newNode);
    }

    /**
     * compare two elements
     * @param key1 first element
     * @param key2 second element
     * @return 0 if elements are equals, <0 if key1< key2 and >0 if key1>key2
     */
